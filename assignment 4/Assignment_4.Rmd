...
mirt_trait_out <- fscores(irt_1pl, method = 'EAP', full.scores = F, full.scores.SE = T)
...
trait_20 <- mirt_trait[which(sum_score==20 & complete.cases(mirt_trait[,c(1:33)])),]
head(trait_20[,c(34:36)])
```  
  
  
The results show that students who got 20 true score have the same trait, $\theta = .402$. Therefore, we need to select 10 items to have the maximum information on the $\theta = .402$.  
  
Next, I try to get the information values of each item on $\theta = .402$ and sort these values in decreasing order. Finally, select the first ten items to make the shorter test.  
...
# fit 1PL on this short test
spec <- 'F = 1-10
CONSTRAIN = (1-10, a1)'

irt_1pl_short <- mirt(df_short, model = spec, itemtype = "2PL", SE=T)
irt_1pl_short
```  
...
```{r}
# get the estimated person trait
mirt_trait_out <- fscores(irt_1pl_short, method = 'EAP', full.scores = F, full.scores.SE = T)
mirt_trait <- as.data.frame(mirt_trait_out)
mirt_trait <- round(mirt_trait[which(complete.cases(mirt_trait)),c(11:12)],2)
head(mirt_trait)
head(mirt_trait[which(mirt_trait$F == .40),])
```
